...
부모 컨테이너에 `display: grid`로 그리드를 지정해주면, 그리드 안에 들어있는 자식 요소들은 모두 그리드의 셀로 변환된다.
...
그리드 레이아웃 방식에는 여러가지가 있지만, 여기서는 Grid Areas로 레이아웃을 구성하는 방법을 알아본다.
...
지정해둔 템플릿 영역 이름을 통해 어떤 영역에 보여질 것인지 지정할 수 있다.
